...
*/
		let miniSPX = IBContract.future("ES", currency: "USD", expiration: try! Date.futureExpiration(year: 2023, month: 3), exchange: .CME)
/*:
...
*/
		let microSPX = IBContract.future(localSymbol: "MESH3", currency: "USD", exchange: .CME)
...
*/
		let callTSLA = IBContract.call("TSLA", currency: "USD", expiration: try! Date.optionExpiration(year: 2023, month: 3, day: 18), strike: 200.0, size: 100, exchange: IBExchange.BOX)
/*:
...
*/
		let callAAPL = IBContract.option(localSymbol: "AAPL  230317C00150000", currency: "USD", exchange: .SMART)
